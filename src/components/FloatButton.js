...
export default function FloatButton() {
  ...
  const [numeroCuentas, setNumeroCuentas] = React.useState(cuentas.length);
  ...
  const crearMovimiento = () => {
    if (amount <= 0) {
      ...
      if (ingreso) { //Si el nuevo movimiento es un ingreso
        var userdata = JSON.parse(sessionStorage.getItem("userData"));

        if (userdata.distribution == 0) {
          var myHeaders = new Headers();
          ...
          cuentas.map(cuentaSeleccionada => {
            var raw = JSON.stringify({
              name: name,
              description: description,
              idCuenta: cuentaSeleccionada.id,
              amount: amount * (cuentaSeleccionada.XXXXX[0].distribution / 100)
            });
            //console.log(raw);
            var requestOptions = {
              method: "POST",
          ...
          setError(true);
          setMensajeError(
            "Quedan ingresos sin asignar, repartalos antes de crear un nuevo ingreso"
          );
        }
      }else{//si es un gasto
        var userdata = JSON.parse(sessionStorage.getItem("userData"));
        ...
        <Grid container spacing={3} className={classes.grid} justify="center">
          <Grid item xs={2} justify="center">
            <Avatar
              ...
              style={{ backgroundColor: elegircolor(selectedColor) }}
            >
